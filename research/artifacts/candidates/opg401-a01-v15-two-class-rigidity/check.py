...
MAPS = INP["maps"]
DECLARED = [set(x) for x in INP["declared_preserved_classes"]]
...
def generic_homs(D, fixed=None):
    ...
    def rec(ds):
        ...
        if all(len(x) == 1 for x in ds):
            sols.append(tuple(next(iter(x)) for x in ds))
            return
        u = min(
            (i for i in range(20) if len(ds[i]) > 1),
            key=lambda i: (len(ds[i]), -len(adj[i]), i),
        )
        for x in sorted(ds[u]):
            nd = [set(z) for z in ds]
            nd[u] = {x}
            rec(nd)
...
def injective_embeddings(D):
    ...
    for u, v in E:
        adj[u].add(v)
        adj[v].add(u)
    assign, used, sols, nodes = {0: 0}, {0}, [], 0
    ...
    def candidates(v):
        return [
            ...
            for x in range(20)
            if x not in used
            and all(u not in assign or edge(x, assign[u]) for u in adj[v])
        ]
    ...
    def rec():
        ...
        nodes += 1
        if len(assign) == 20:
            sols.append(tuple(assign[i] for i in range(20)))
            return
        ...
        v = min(
            ...
            key=lambda z: (
                len(candidates(z)),
                -sum(u in assign for u in adj[z]),
                z,
            ),
        )
...
def independent_counts(D):
    ...
    return (
        sum(ok(C) for C in combinations(range(20), 7)),
        sum(ok(C) for C in combinations(range(20), 8)),
    )
                                ...
                                if edge(b, c):
                                    for a in A[P]:
                                        us = [
                                            u
                                            for u in range(20)
                                            if edge(a, u) and edge(u, b)
                                        ]
                                        vs = [
                                            v
                                            for v in range(20)
                                            if edge(a, v) and edge(v, c)
                                        ]
                                        if us and vs:
                                            return (a, us[0], b, c, vs[0], d, e, f)
    return None
...
def mixed_exists(colors, edges):
    for opts in product(range(3), repeat=len(colors)):
        if all(
            edge(MAPS[opts[u]][colors[u]], MAPS[opts[v]][colors[v]])
            for u, v in edges
        ):
            return opts
    return None

# A0. Positive source-faithfulness tests for all three corrected V14 maps.
positive_map_checks = 0
for T, D in zip(MAPS, DECLARED):
    for c in range(20):
        for d in D:
            for sign in (-1, 1):
                assert edge(T[c], T[(c + sign * d) % 20])
                positive_map_checks += 1

# The stale branch word must be caught by those positive tests.
stale = INP["correction"]["rejected_stale_word"]
stale_failures = []
for c in range(20):
    for d in (7, 8):
        for sign in (-1, 1):
            if not edge(stale[c], stale[(c + sign * d) % 20]):
                stale_failures.append([c, d, sign])
assert stale_failures

# A1. Two-class global-map rigidity, by two code paths.
    ...
    pair_rows.append(
        {
            "distances": sorted(D),
            "alpha": 7,
            "max_independent_count": n7,
            "size8_independent_count": n8,
            "generic_nodes": nodes,
            "injective_nodes": inj_nodes,
            "normalized_maps": [list(x) for x in sols],
            "t7_table": t7,
        }
    )
...
cert_rows = {
    (r["p"], *r["terminal"]): (r["Q"], r["R"], tuple(r["O"]))
...
# C. Exact minimum path obstruction and every ordered length-two obstruction.
bad = []
for a, b, c in product(range(20), repeat=3):
    if (
        edge(a, b)
        and edge(b, c)
        and mixed_exists([a, b, c], [(0, 1), (1, 2)]) is None
    ):
        bad.append([a, b, c])
...
for m, c, d in [
    ...
    assert edge(c, (c + d) % 20)
    assert not edge(MAPS[m][c], MAPS[m][(c + d) % 20])
    mut += 1
for middle in range(3):
    assert not (middle in left and middle in right)
    mut += 1
assert mixed_exists([19, 8, 1], [(0, 1), (1, 2)]) is not None
mut += 1
assert mixed_exists([18, 8, 0], [(0, 1), (1, 2)]) is not None
mut += 1
assert all(
    valid_o((P, q, r, S), ow)
    for (p, X, Y, y, P, S), (q, r, ow) in unique.items()
...
badw = list(ow)
badw[0] = (badw[0] + 1) % 20
assert not valid_o((P, q, r, S), badw)
mut += 1
assert delta(19, 8) == 9 and delta(8, 0) == 8
mut += 1
assert len(set(tuple(x) for x in bad)) == 30
mut += 1
assert mut == 15
...
print(
    json.dumps(
        {
            "status": "ok",
            "positive_map_checks": positive_map_checks,
            "stale_T8_failures": len(stale_failures),
            "pair_graphs": 3,
            "generic_nodes": sum(r["generic_nodes"] for r in pair_rows),
            "injective_nodes": sum(r["injective_nodes"] for r in pair_rows),
            "max_independent_counts": [
                r["max_independent_count"] for r in pair_rows
            ],
            "terminal_option_rows": option_rows,
            "terminal_unique_states": len(unique),
            "unsat_ordered_paths": len(bad),
            "minimal_path_vertices": 3,
            "mutations": mut,
